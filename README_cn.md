...
| `libs/ml/` | **机器学习**——Rust cdylib + FFI 本地训练/推理 | classifier（TF-IDF + softmax 头 + 温度校准 + Clopper-Pearson 阈值推荐） |
| `libs/entity/` | **实体解析**——记录链接管道 | entity（blocking+相似度+连通分量聚类） |
| `libs/privacy/` | **隐私工程**——差分隐私/脱敏/k-匿名/临床日期平移/去标识化效果评估/自由文本 PHI 脱敏/凭据脱敏/可逆还原 | privacy（dp_count/dp_sum/dp_mean、dp_compose/dp_alloc/dp_budget、mask、mask_cn、dateshift、redact_text、redact_secrets、restore_text、kanon、kanon_report） |
| `libs/security/` | **安全**——策略与审计原语 | rbac（Quack RBAC 策略引擎）、audit_chain（链式哈希审计，防篡改） |
...
| `libs/ml/classifier.lua` | ml | jimothy 模式文本分类器（Rust cdylib FFI 内核 + 薄 Lua 绑定）：`train`（中英混合 bigam 分词 + TF-IDF + 线性 softmax 头，full-batch Adam，L2 网格 {1e-3..0}；自动或显式校准切分；校准集上做温度缩放；`out:` 落盘自包含模型 JSON）、`predict`（批量文本 → label+probability+probs，可选 `threshold` 门控 → `accepted`）、`evaluate`（accuracy + 每类 P/R + 混淆矩阵）。report 自带 Clopper-Pearson 精确二项下界阈值网格（13 档，只推荐不强制——校准集太小返回 `insufficient_data`，与 jimothy 同款诚实）。错误一律 `{"status":"Error","message":...}` JSON。需 `libclassifier_capi.so` 在 `~/.duckdb/luajit-libs/` 或 `LD_LIBRARY_PATH`（源码 `/mnt/d/wsl2/classifier_capi`，`cargo build --offline --release`） | libclassifier_capi.so |
...
| `libs/privacy/privacy.lua` | privacy | 隐私工程原语（纯 Lua 自包含）：`dp_count`/`dp_sum`/`dp_mean`（ε-差分隐私，Laplace 机制，顺序组合线性预算；Park-Miller LCG Schrage 法——double 精确无 2^53 溢出，seed 可复现）、`laplace`（机制暴露）、`mask`（hash 盐化 FNV-1a(mul32 精确回绕)/star/bin 泛化/suppress/rand 确定性替换）、**`mask_cn`（CN 合规脱敏规则库：身份证前 6 后 4 / 手机号前 3 后 4 / 银行卡前 6（BIN）后 4 / 姓名保留姓（复姓识别）/ 邮箱保留首字符+域名；`kind:'auto'` 自识别；`mode:'star'` 格式保持、`'hash'` 确定性指纹可作外键、`'birth'` 身份证生日泛化到年；识别失败 fail-closed 退通用 star）**、**`dateshift`/`dateoffset`（临床 MIMIC 式日期平移：偏移 = FNV-1a(key) mod (2·days+1) − days 只依赖 subject 键 → 同 subject 恒定偏移，住院时长/事件间隔逐位不变，绝对日期不可反推；Howard Hinnant civil-days 纯整数，闰年精确——实测 2,055 例覆盖 1900–2200 全区间（含全部闰日）与 Python datetime 逐例一致；DuckDB 端 12 项断言全过：零泄漏、外键哈希连接行数不变、同 subject 偏移漂移 0、|偏移| ≤ days、住院时长与相对窗口平移前后集合相等）**、`kanon`（k-匿名 Mondrian 简化：等权范围分裂 + 区间/前缀泛化）、**`kanon_report`（GB/T 42460 去标识化效果评估报告：等价类内 distinct-l 与 entropy-l（判据 H ≥ log₂l）、t-closeness（分类属性用总变差 TV；`ordered:true` 且敏感属性为数值时用归一化 1-D EMD（Wasserstein-1），非数值则诚实回退 TV）、抑制率、按组大小加权的简化 ILA 泛化损失（数值维度取跨度比、分类维度取组内离散度）；输出 k/l/t 三项判定 + 显式 `violations` 清单，且报告里的聚合字段在 CI 中会由其自带分组明细再聚合一遍做独立交叉校验）**、**`dp_compose`（组合界：basic 精确 Σε；Dwork-Roth 强组合界 Σεᵢ(e^εᵢ−1) + √(2·ln(1/δ′)·Σεᵢ²)）**、**`dp_alloc`（把总 ε 预算切给 n 个计划查询：basic 均分，或二分求强组合界下的最大均匀 ε；例：预算 1.0、n=100、δ′=1e-5 → 单查询 ε 从 0.01 提到 0.019998）**、**`dp_budget`（单步 ε 台账门禁：传 `ledger` 历史消耗数组和/或标量 `spent`，返回 allow/deny 与消耗前/后、余额前/后；超预算 reason='budget_exceeded'；`composition:'advanced'` 时按强组合界判定，故 100 笔 ε=0.01 记为 0.4899 而非 1.0——并如实报出小 n 时强组合界反而更松的情形）**、**`redact_text`（自由文本 PHI 占位符式脱敏，MIMIC `[**Name1**]` 风格：规则表驱动 + 调用方字典；覆盖邮箱、URL、IPv4（逐八位组校验）、日期四形态（`-`/`/`/`.`/`年月日`，校验月日合法）、身份证（15/18/17+X）、手机（`1[3-9]` + 9 位）、银行卡（16–19 位），以及未分类长数字串的保守兜底（≥ `num_min`，默认 9 位，故临床短数字不动）；字典按**字典顺序**编号而非行内首现序 ⇒ 同一人跨行恒得同一编号（稳定假名）；给了 `key` 则日期按与 `dateshift` 完全相同的偏移平移（默认盐一致），结构化列与自由文本共用同一时间轴，未给 `key` 则置 `[**DATE**]`；幂等；返回逐规则计数，并带一句诚实声明——规则驱动只覆盖列出的模式，未匹配的自由文本不保证无 PHI）**、**`redact_secrets`（机器侧凭据/拓扑脱敏，redact_text 管 PII/PHI、本 op 管机器侧敏感物——表驱动厂商凭据形态：GitHub `ghp_/gho_/…`、`github_pat_`、Google `AIza`、阿里云 `LTAI`、腾讯云 `AKID`、AWS `AKIA/ASIA`、Slack `xox[baprs]`、Stripe `sk_live_/sk_test_`、飞书 `cli_`、钉钉 `ding`、OpenAI `sk-`、Azure `ah-`、JWT（`eyJ` 三段），外加 Bearer token 值、连接串密码（`scheme://user:***@host` 只掩密码段）、统一社会信用代码 USCC（GB 32100-2015 MOD31 校验位压误报）、内网 IPv4（10/172.16-31/192.168/169.254/CGNAT 100.64-127）；占位符风格与词边界纪律同 redact_text，版本号（`v2.0.1.0`）、五段编号、包名不误掩；`p.extra` 收调用方自研 token 前缀；诚实 `note` 声明未收录形态需 `extra` 补）**、**`restore_text`（redact_text 的可逆对称 op——传脱敏文本 + redact_text 返回的 `map`，字典占位符还原回原文；key/日期平移占位符按同一 `key`+`days` 回移；**无可还原来源的占位符在输出里原样可见并计入 `unresolved`——visible-failure 契约，绝不静默吞掉漏掉的映射，与 maskit 同纪律**）** | 无 |
| `libs/security/audit_chain.lua` | security | 链式哈希审计（防篡改审计日志；纯 Lua + DuckDB 内建，零新依赖）：`setup` 建表 `<t>(seq BIGINT, ts VARCHAR, prev_hash, hash, actor, action, scope, scope_hash, payload, outcome)`；`append` 写入一条记录，`hash = sha256(concat_ws('|', prev_hash, CAST(seq AS VARCHAR), ts, actor, action, scope, scope_hash, payload, outcome))`，`prev_hash` = 上一行 hash（创世行为空串）；`verify` **一条 SQL 同时复算每行 hash 与链指针**（对存储列求 `sha256(...)` + `lag(hash) OVER (ORDER BY seq)`），返回 `{ok, n, first_bad, reason}`，`reason = hash_mismatch | chain_break`；另有 `head`、`logs`、`scope_hash`（对授权范围求稳定指纹——与 dataapp 的 `paramsHash` 同一角色）、`status`。PoC 7/7：干净链 `ok:true`；改一条历史 `payload` → `first_bad=2 hash_mismatch`；还原 → 又 `ok:true`；只改链指针 → `first_bad=3`；**删中间行 → `first_bad=3 chain_break`**；删**尾部**行 → `ok:true`（链本身看不见截断，须把 `n` 与外部台账对账）。规范串写在库头，任何语言都能独立复算；测试里三行 hash 用**纯 SQL、不经 Lua** 重算全部 `match_sql = true`。诚实边界：它保证记录「事后可验证未被改动」，**不保证写入当时不可伪造**——持有写权限者可重算整条链（要闭环需配 rbac 的 `allow_write=false`） | `_duckdb_query`（普通模式）+ 内建 `sha256()` |
