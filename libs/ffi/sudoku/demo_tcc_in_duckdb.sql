-- demo_tcc_in_duckdb.sql — 无 gcc 环境:在 duckdb-luajit 扩展内,
-- 用 libtcc 运行时编译 sudoku_solve.c 并注册 UDF(零预编译,读源码即跑)
--
-- 前置:
--   1. 本机有 tcc (sudo apt install tcc; 或 ~/.local/tcc)
--   2. 修改下方 REPO_ROOT / TCC_PREFIX 为你的路径
-- 运行:
--   duckdb -unsigned < demo_tcc_in_duckdb.sql
-- 预期输出: compile OK + correct = true
LOAD '/mnt/d/wsl2/luajit/build/release/luajit.duckdb_extension';

-- 模块 body 只编译一次,函数指针缓存;UDF 每次调用直接求解
SELECT * FROM luajit_module(mode := 'compile', sql_name := 'sudoku_tcc', source := '
...
local f = assert(io.open("/mnt/d/wsl2/duckdb-luajit-libs/libs/ffi/sudoku/sudoku_solve.c", "rb"))
...
WITH got AS (
  SELECT luajit_s('sudoku_tcc', '000001002000020030004500600007600050080090006100005800001004000070900003400030020') AS sol
)
