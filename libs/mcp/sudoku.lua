...
-- 性能（2026-08-08 实测，sudoku17 同题集 100 题，修正历史虚高数字）:
--   v1 naive 回溯 224.4ms/题 → v2 0.13ms/题 ≈ 477x（纯算法口径；逐行 SQL 调用 378x）
--   ⚠️ 9000x 是题目集不对等（最难前 20 vs 混合前 100）的虚高值，勿引用
--   C(gcc -O3) 0.019ms/题、Rust(cdylib) 0.021ms/题（同算法，FFI 直调）— 比 Lua 快 ~7 倍
local sudoku = {}
